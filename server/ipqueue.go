...
type ipQueue[T any] struct {
	...
	calc func(e T) int // Per-entry size calculator
	sz   int           // Calculated size (only if calc != nil)
	name string
	m    *sync.Map
}
...
func ipQueue_SizeCalculation[T any](calc func(e T) int) ipQueueOpt[T] {
	return func(o *ipQueueOpts[T]) {
		o.calc = calc
	}
}
