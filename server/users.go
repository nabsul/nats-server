...
func (s *Server) fetchUser(opts ClientOpts) string {
	if opts.JWT != _EMPTY_ {
		return opts.JWT
	}
	...
	userLookupRequest := fmt.Sprintf(usrLookupReqSubj, nkey)
	s.Noticef("Fetching user %q with subject %q", nkey, userLookupRequest)
	s.mu.Lock()
	...
	s.mu.Unlock()
	var err error
	s.Noticef("Fetching user %q", nkey)
	var theJWT string
	select {
	case <-quit:
		s.Noticef("Fetching user %q failed due to shutdown", nkey)
		err = errors.New("fetching user jwt failed due to shutdown")
	case <-time.After(timeout):
		s.Noticef("Fetching user %q timed out", nkey)
	...
	s.mu.Unlock()
	close(respC)
	s.Noticef("Fetched user %q and received JWT %q", nkey, theJWT)
